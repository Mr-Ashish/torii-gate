...
def assess(root: Path | None = None, *, deep: bool = True) -> dict[str, Any]:
    ...
    return {
        ...
        # F170: GEPA refine compound loop (F165–F169) one product readiness bit
        "refine_loop_ok": bool(
            # F165 refine-from-util wired
            "refine_from_util"
            in (
                (root / "scripts" / "self_evolve.py").read_text(
                    encoding="utf-8", errors="replace"
            ...
            and "refine-from-util" in hermes_sh
            and "F165" in hermes_sh
            # F166 dual-gate LOO + fitness shield
            and "refine_attr_enabled"
            in (
                (root / "scripts" / "skill_attribution.py").read_text(
                    encoding="utf-8", errors="replace"
                )
            ...
            and "ingest-refine" in hermes_sh
            and "F166" in hermes_sh
            # F167 dual-rollout contribution_pp
            and "run_refine_dual"
            in (
                (root / "scripts" / "skill_dual_rollout.py").read_text(
                    encoding="utf-8", errors="replace"
            ...
            and "refine-dual" in hermes_sh
            and "F167" in hermes_sh
            # F168 multi-tenant promote
            and "promote_refine_dual_themes"
            in (
                (root / "scripts" / "skill_dual_rollout.py").read_text(
                    encoding="utf-8", errors="replace"
                )
            ...
            and "promote-refine-dual" in hermes_sh
            and "F168" in hermes_sh
            # F169 hub inject + dual_fail critic
            and "post_score_refine_dual_hub"
            in (
                (root / "scripts" / "skill_router.py").read_text(
                    encoding="utf-8", errors="replace"
                )
            ...
            and "inject_refine_dual_hub_into_prompt"
            in (
                (root / "scripts" / "skill_router.py").read_text(
                    encoding="utf-8", errors="replace"
                )
            ...
            and "refine_dual_fail_idle_approve"
            in (
                (root / "scripts" / "second_agent_critic.py").read_text(
                    encoding="utf-8", errors="replace"
                )
            ...
            # F181 hub×GEPA compound prompt inject
            and "inject_hub_gepa_compound_into_prompt"
            in (
                (root / "scripts" / "skill_router.py").read_text(
                    encoding="utf-8", errors="replace"
                )
            ...
            and "F181" in hermes_sh
            # F182 hub×GEPA compound always priority
            and "hub_gepa_compound_always_enabled"
            in (
                (root / "scripts" / "skill_router.py").read_text(
                    encoding="utf-8", errors="replace"
                )
            ...
            and "hub_gepa_compound_priority_deltas" in (
                (root / "scripts" / "skill_router.py").read_text(
                    encoding="utf-8", errors="replace"
                )
                if (root / "scripts" / "skill_router.py").is_file()
        ...
        # F181: prompt inject of hub×GEPA compound pressure
        "hub_gepa_compound_inject_ok": "inject_hub_gepa_compound_into_prompt"
        in (
            (root / "scripts" / "skill_router.py").read_text(
                encoding="utf-8", errors="replace"
            )
        ...
        and "assess_hub_gepa_compound" in (
            (root / "scripts" / "skill_router.py").read_text(
                encoding="utf-8", errors="replace"
            )
            if (root / "scripts" / "skill_router.py").is_file()
        ...
        and "F181" in hermes_sh
        and "hub_gepa_compound_injected" in (
            (root / "scripts" / "skill_router.py").read_text(
                encoding="utf-8", errors="replace"
            )
            if (root / "scripts" / "skill_router.py").is_file()
        ...
        # F182: hub×GEPA compound always-priority budget
        "hub_gepa_compound_always_ok": "hub_gepa_compound_always_enabled"
        in (
            (root / "scripts" / "skill_router.py").read_text(
                encoding="utf-8", errors="replace"
            )
        ...
        and "F182" in hermes_sh
        and "feature_hub_gepa_compound_always" in (
            (root / "scripts" / "skill_router.py").read_text(
                encoding="utf-8", errors="replace"
            )
            if (root / "scripts" / "skill_router.py").is_file()
        ...
        json.dumps(
            {
                ...
                or "F180",
                "hub_gepa_compound_inject_ok": report.get("hub_gepa_compound_inject_ok"),
                "feature_hub_gepa_compound_inject": report.get(
                    "feature_hub_gepa_compound_inject"
                )
                or "F181",
                "hub_gepa_compound_always_ok": report.get("hub_gepa_compound_always_ok"),
                "feature_hub_gepa_compound_always": report.get(
