...
#
# Env:
#   HERMES_HOME (or TORII_ROOT/.torii-hermes-home)
...
[[ -f "$MEMORY_FILE" ]] || cp -f "$TORII_ROOT/agent/MEMORY.seed.md" "$MEMORY_FILE" 2>/dev/null || \
...
text = path.read_text(errors="replace") if path.exists() else "# Torii Gate review memory\n"
text = text.rstrip() + "\n" + block
