...
| **GEPA refine loop** | refine → dual-gate → dual_pp → promote → hub critic → chronic decay → multi-tenant demote → dual_pass revive + free-rider MT + contribution_pp floor — one measured `refine_loop_ok` bit (F165–F177 / F178 pack). |
