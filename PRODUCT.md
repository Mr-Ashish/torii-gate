...
**Compound memory (F93–F94).** Write-path Mem0-style events (ADD/UPDATE/DELETE/NONE): path-anchored FP can supersede overlapping TP so deleted noise does not resurface. Maintenance pass consolidates with **importance × half-life decay**, merges near-dup themes, and evicts stale low-value items so scoped recall stays sharp. FP rules, TP signatures, and privacy-safe federated themes compound across PRs and tenants.
